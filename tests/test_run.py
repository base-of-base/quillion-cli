...
class TestRunCommand:
    ...
    def mock_debugger(self):
        with patch("quillion_cli.commands.run.debugger.banner") as mock_banner, patch(
            "quillion_cli.commands.run.debugger.error"
        ) as mock_error, patch(
            "quillion_cli.commands.run.debugger.info"
        ) as mock_info, patch(
            "quillion_cli.commands.run.debugger.server_start"
    ...
    def mock_servers(self):
        with patch("quillion_cli.commands.run.run_server") as mock_run_server, patch(
            "quillion_cli.commands.run.start_http_server"
        ) as mock_http_server, patch(
            "quillion_cli.commands.run.restart_server"
        ) as mock_restart_server, patch(
            "quillion_cli.commands.run.setup_file_watchers"
        ) as mock_watchers, patch(
            "quillion_cli.commands.run.shutdown_server"
        ) as mock_shutdown_server, patch(
            "quillion_cli.commands.run.shutdown_watchers"
        ) as mock_shutdown_watchers:
    ...
    def test_run_command_with_project(self, mock_config, mock_debugger, mock_servers):
        with patch(
            "quillion_cli.commands.run.load_config", return_value=mock_config
        ), patch(
            "quillion_cli.commands.run.start_development_server"
        ) as mock_start_dev:
    ...
    def test_run_command_config_overrides(
        self, mock_config, mock_debugger, mock_servers
    ):
        with patch(
            "quillion_cli.commands.run.load_config", return_value=mock_config
        ), patch(
    ...
    def test_start_development_server_no_hot_reload(
        self, mock_config, mock_debugger, mock_servers
    ):
        with patch("time.sleep", side_effect=KeyboardInterrupt()):
            ...
            mock_servers["run_server"].assert_called_once_with(
                mock_config, "/test/project"
            )
            mock_servers["process"].wait.assert_called_once()

    def test_start_development_server_with_hot_reload(
        self, mock_config, mock_debugger, mock_servers
    ):
        ...
        with patch("time.sleep", side_effect=KeyboardInterrupt()):
            ...
            mock_servers["shutdown_server"].assert_called_once_with(
                mock_servers["process"]
            )
            mock_servers["shutdown_watchers"].assert_called_once_with(
                mock_servers["observers"]
            )

    def test_start_development_server_process_dies(
        self, mock_config, mock_debugger, mock_servers
    ):
        ...
        with patch("time.sleep", side_effect=[None, KeyboardInterrupt()]):
            ...
            mock_servers["restart_server"].assert_called_once_with(
                mock_config, "/test/project", mock_servers["process"]
            )
    ...
    def test_start_development_server_debugger_calls(
        self, mock_config, mock_debugger, mock_servers
    ):
        with patch("time.sleep", side_effect=KeyboardInterrupt()):
            from quillion_cli.commands.run import start_development_server
    ...
    def test_run_command_current_directory(
        self, mock_config, mock_debugger, mock_servers
    ):
        with patch(
            "quillion_cli.commands.run.load_config", return_value=mock_config
