...
class TestHTTPRequestHandler:
    ...
    def test_translate_path_directory_without_index(self, temp_project_dir):
        ...
        with patch.object(http.server.SimpleHTTPRequestHandler, 'translate_path') as mock_parent:
    ...
    def test_translate_path_custom_packages_dir(self, temp_project_dir):
        ...
        handler = self.create_handler_instance(temp_project_dir, packages_dir="custom_pkg")
    ...
    def test_end_headers_adds_cors(self, temp_project_dir):
        ...
        with patch.object(handler, 'send_header') as mock_send_header:
            with patch.object(http.server.SimpleHTTPRequestHandler, 'end_headers') as mock_parent_end:
                handler.end_headers()
                
                mock_send_header.assert_any_call("Access-Control-Allow-Origin", "*")
                mock_send_header.assert_any_call("Access-Control-Allow-Methods", "GET, POST, OPTIONS")
                mock_send_header.assert_any_call("Access-Control-Allow-Headers", "Content-Type")
    ...
    def test_guess_type_delegation(self, temp_project_dir):
        ...
        with patch.object(http.server.SimpleHTTPRequestHandler, 'guess_type') as mock_guess:
...
class TestStartHTTPServer:
    ...
    def test_start_http_server_missing_packages_dir(self, temp_project_dir, mock_config):
    ...
    def test_start_http_server_with_ssl(self, temp_project_dir, mock_config):
        ...
        with patch('quillion_cli.server.http_server.ssl.SSLContext') as mock_ssl_context:
            mock_context = Mock()
    ...
    def test_start_http_server_port_in_use(self, temp_project_dir, mock_config):
        ...
        with patch('quillion_cli.server.http_server.socketserver.TCPServer') as mock_tcpserver:
    ...
    def test_start_http_server_directory_change_restored(self, temp_project_dir, mock_config):
    ...
    def test_server_exception_handling(self, temp_project_dir, mock_config):
        ...
        with patch('quillion_cli.server.http_server.threading.Thread') as mock_thread_class:
